...
public class ColliderGenerator : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        for (int i = 0; i < resolution + 1; i++)
        {
            ...
            {
                Vector3 position = new Vector3(i - (resolution + 1)/2.0f, 1, v - (resolution + 1)/2.0f);
                float displacement = _cpuTexture.GetPixelBilinear(-position.x/((resolution + 1)/2.0f) * 0.5f + 0.5f, -position.z/((resolution + 1)/2.0f) * 0.5f + 0.5f).r;
